...
/* Dumps the locations where each task was first recorded being completed. */
class cActionDumpFirstTaskLocations : public cAction
{
private:
  cString m_filename;

public:
  cActionDumpFirstTaskLocations(cWorld* world, const cString& args, Feedback&) : cAction(world, args), m_filename("")
  {
    cString largs(args);
    if (largs.GetSize()) m_filename = largs.PopWord();
  }
